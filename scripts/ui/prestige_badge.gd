...
func _draw() -> void:
	...
	# Il livello, allineato a destra sopra alla barra, con l'ombra dura come
	# tutte le scritte dell'HUD.
	var text_w := font.get_string_size(_level, HORIZONTAL_ALIGNMENT_LEFT, -1, 12).x
	var at := Vector2(size.x - text_w, y_bar - 5.0)
	draw_string(font, at + Vector2(1, 1), _level, HORIZONTAL_ALIGNMENT_LEFT, -1, 12,
		Color(0, 0, 0, 0.7))
	draw_string(font, at, _level, HORIZONTAL_ALIGNMENT_LEFT, -1, 12, TEXT)
