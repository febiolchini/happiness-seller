...
## Nomi mostrati a schermo, nell'ordine dell'enum. Solo lettere, quindi si
## possono scrivere col font del gioco.
const STAGE_NAMES := ["EMPTY", "SEEDLING", "VEGETATIVE", "FLOWERING", "READY"]
...
##
## Va letta insieme a `grow_hours` della varietà (30) e a
...
static func plant(plot: Dictionary, strain_id: String, now: float) -> void:
	plot.clear()
...
static func grow_hours(plot: Dictionary) -> float:
	return maxf(1.0, float(Economy.strain(strain_of(plot))["grow_hours"]))
...
static func stage_name(plot: Dictionary, now: float) -> String:
	return STAGE_NAMES[stage(plot, now)]
...
static func yield_grams(plot: Dictionary) -> int:
	if plot.is_empty():
		return 0
	var base := float(Economy.strain(strain_of(plot))["grams"])
	return maxi(1, int(roundf(base * quality(plot))))
